...
class Catalog {
  ...
  Catalog copyWithField(String field, String value) {
    switch (field) {
      case 'Продукт':
        return copyWith(product: value);
      case 'Артикул':
        return copyWith(article: value);
      case 'Код':
        return copyWith(code: value);
      case 'Терминология':
        return copyWith(terminology: value);
      case 'Путь':
...
class _ProductCodesState extends State<ProductCodes> {
      ...
      if (result != null) {
        ...
        setState(() {
          if (index != null) {
            ...
            _orders.add(Catalog(
              product: fileName,
              article: 'Текущий пользователь',
              code: _formatDate(DateTime.now()),
              terminology: 'Текущий пользователь',
              filePath: file.path,
            ));
          }
        });
  ...
  void _exportToCsv() async {
    try {
      String content = 'Продукт;Артикул;Код;Терминология;Путь\n';
          
      for (var order in _orders) {
        content += '${order.product};${order.article};${order.code};${order.terminology};'
            '${order.filePath}\n';
      }
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                setState(() {
                  _orders.add(Catalog(
                    product: nameController.text.isNotEmpty ? nameController.text : 'Новый документ',
                    article: 'Текущий пользователь',
                    code: formattedDate,
                    terminology: 'Текущий пользователь',
                    filePath: '',
